...
const BlogDetail = () => {
    ...
    if (error) return <div className="text-center text-red-500">Error loading blog</div>

    const blog = data?.blogPosts.find(
        (post) => sanitizeTitle(post.title) === title
    )

    if (!blog) return <div className="text-center text-muted py-20">Post not found</div>
